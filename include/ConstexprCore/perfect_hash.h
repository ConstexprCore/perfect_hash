...
struct perfect_hash_set {
    ...
    std::array<std::uint8_t, TableSize> slot_key_len_{};                    // 0xFF = empty
    std::array<std::array<char, MaxKeyLen>, TableSize> slot_key_data_{};
    std::array<word_type, TableSize> slot_key_word_{};                      // pre-packed for word compare
    // --- cold data ---
    std::array<std::uint8_t, TableSize> slot_to_key_{};
