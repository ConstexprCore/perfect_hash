#!/usr/bin/env python3
"""Check that perfect_hash can actually be consumed the three documented ways.

  1. drop-in      — copy singleheader/perfect_hash.h next to your code, compile
  2. FetchContent — add_subdirectory/FetchContent, link ConstexprCore::perfect_hash
  3. find_package — install the project, then find_package(perfect_hash)
...
SINGLEHEADER = PROJECT_ROOT / "singleheader" / "perfect_hash.h"
...
def test_find_package(workdir: Path) -> None:
    ...
    installed = prefix / "include" / "ConstexprCore" / "perfect_hash.h"
    if not installed.is_file():
        raise SystemExit(f"expected {installed} to be installed")
    if "fixed_string" not in installed.read_text(encoding="utf-8"):
        raise SystemExit("installed header does not look amalgamated (no bundled fixed_string)")
