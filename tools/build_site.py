#!/usr/bin/env python3
"""Build the perfect_hash project page from README.md.
...
REPO_URL = "https://github.com/ConstexprCore/perfect_hash"
SITE_URL = "https://constexprcore.github.io/perfect_hash/"

HERO_CODE = '''#include "perfect_hash.h"
...
// Table built at compile time. Lookup is a
// handful of instructions, zero collisions.
std::optional<Method> parse(std::string_view s) {
...
def split_readme(text: str) -> tuple[str, str, str]:
    ...
    for index in range(1, len(lines)):
        ...
        if not tagline and not line.startswith("#"):
            tagline = line
            continue
        if line.startswith("## "):
            body_start = index
...
def headline_numbers(body: str) -> list[tuple[str, str]]:
    """Pull the top benchmark row out of the README so the strip cannot go stale."""
    ours = None
    for line in body.splitlines():
        match = re.match(r"\|\s*\*\*ours\*\*\s*\|\s*([0-9.]+)\s*\|", line)
        if match:
            ours = match.group(1)
            break
    stats = [
        (f"{ours} B/s" if ours else "1 B/s", "lookups per second, single core"),
        ("0", "collisions, by construction"),
        ("1", "header, no dependencies"),
        ("C++23", "header-only, consteval"),
    ]
    return stats
...
def build(output: Path) -> Path:
    ...
    replacements = {
        ...
        "REPO_URL": REPO_URL,
        "SITE_URL": SITE_URL,
        "DOWNLOAD_URL": f"{REPO_URL}/releases/download/v{release}/perfect_hash.h",
        "TOC": toc,
        "CONTENT": content,
