...
you — the script prints the command.
    ...
    python3 tools/release.py 0.2.0        # explicit version
    python3 tools/release.py --minor      # 0.1.3 -> 0.2.0
    python3 tools/release.py 0.2.0 -n     # dry run: show what would change

Version numbers live in:
    CMakeLists.txt                                  project(... VERSION x.y.z)
    include/ConstexprCore/perfect_hash_version.h    macros + constants
    README.md                                       pinned download URLs
    singleheader/perfect_hash.h                     regenerated banner
"""
...
VERSION_HEADER = PROJECT_ROOT / "include" / "ConstexprCore" / "perfect_hash_version.h"
...
def write_readme(version: str, dry_run: bool) -> bool:
    ...
    updated = re.sub(r"(perfect_hash/)v\d+\.\d+\.\d+(/singleheader/)", rf"\g<1>v{version}\g<2>", updated)
...
def regenerate_singleheader(dry_run: bool) -> bool:
    if dry_run:
        ...
        return True
    subprocess.run([sys.executable, str(AMALGAMATE), "--quiet", "--fetch"], check=True)
...
def main() -> int:
    ...
    if not args.no_tag:
        git("tag", "-a", tag, "-m", f"perfect_hash {version}")
        print(f"tagged {tag}")
    ...
    print(f"\nThen attach singleheader/perfect_hash.h to the {tag} GitHub release so the")
